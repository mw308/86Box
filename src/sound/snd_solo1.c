 ...
 *
 *          ESS Technology ES1938S Solo-1 PCI audio controller.
 *
 * Phase 6: restore Solo-1 legacy SB/FM/MPU/gameport decode; keep Phase 5B Audio 2.
 */
...
typedef struct solo1_t {
     ...
     *
     * The initial functional implementation called mem_readb_phys() once
     * for every PCM byte.  That is correct but extremely expensive on a
     * fast late-1990s guest. Cache only a small rolling chunk of ordinary
     * guest RAM. Windows refills auto-init DMA buffers while the device is
     * consuming them, so caching the whole buffer can replay stale samples
     * and cause audible crackle.
     */
    ...
    /* PCI diagnostic read compression. */
    int     last_read_func;
    int     last_read_addr;
    uint8_t last_read_val;
    uint32_t repeated_reads;
} solo1_t;

static void
solo1_trace(const char *fmt, ...)
{
    /* Disabled in phase 4B: the per-PCI-access trace delayed Windows boot
       by several minutes and is no longer useful for the playback work. */
    (void) fmt;
}

static void
solo1_flush_repeated_reads(solo1_t *dev)
{
    if (dev->repeated_reads) {
        solo1_trace("PCI slot=%02X READ %02X -> %02X repeated %u additional times\n",
                    dev->pci_slot, dev->last_read_addr, dev->last_read_val,
                    dev->repeated_reads);
        dev->repeated_reads = 0;
    }
}
...
solo1_pci_read(int func, int addr, UNUSED(int len), void *priv)
{
    ...
    if ((func == dev->last_read_func) &&
        (addr == dev->last_read_addr) &&
        (ret == dev->last_read_val)) {
        dev->repeated_reads++;
    } else {
        solo1_flush_repeated_reads(dev);
        solo1_trace("PCI slot=%02X READ func=%d %02X -> %02X\n",
                    dev->pci_slot, func, addr, ret);
        dev->last_read_func = func;
        dev->last_read_addr = addr;
        dev->last_read_val  = ret;
    }
...
static void
solo1_io_trace(const char *fmt, ...)
{
    /* Phase 5A performance build: runtime logging disabled. */
    (void) fmt;
}
...
solo1_a2_cache_fill(solo1_t *dev)
{
    ...
    /*
     * Windows places the Solo-1 playback buffers in ordinary system RAM.
     * Use 86Box's direct RAM backing store for that common path.  This
     * avoids a memory-map lookup for every PCM byte.
     *
     * If a future guest gives the device an address which is not ordinary
     * RAM, retain correct PCI-bus-master semantics through the physical
     * memory helpers, but read a dword at a time where possible.
     */
...
solo1_get_buffer(int32_t *buffer, uint16_t len, void *priv)
{
    ...
    for (uint16_t i = 0; i < len; i++) {
        ...
        if (active) {
            ...
            /* For the first functional pass, treat non-zero 7Ch as audible.
               Proper ESS logarithmic mixer gain can be added after transport
               and IRQ timing are proven. */
            if (dev->mixer_regs[0x7c] != 0x00) {
                buffer[i * 2]     += dev->a2_last_l;
                buffer[i * 2 + 1] += dev->a2_last_r;
            }
        }
    }
}
    ...
    switch (off) {
        ...
        case 0x06: {
            ...
            if (!(old & 0x02) && (dev->io_regs[0x06] & 0x02)) {
                ...
                dev->a2_rate_accum = 0;
                solo1_io_trace("A2 START addr=%08X count=%u rate=%u fmt=%02X ctl=%02X\n",
                               dev->a2_base_addr, dev->a2_base_count,
                               solo1_a2_sample_rate(dev),
                               dev->mixer_regs[0x7a],
                               dev->mixer_regs[0x78]);
            }
            break;
...
solo1_ess_reset_regs(solo1_t *dev)
{
    ...
    /* Same documented/reset-compatible baseline used by the existing
       AudioDrive core for the transfer-count high byte. */
...
solo1_dsp_command_byte(solo1_t *dev, uint8_t val)
{
    if (dev->dsp_pending_bytes) {
        ...
        if (cmd == 0xC0) {
            ...
            solo1_io_trace("DSP C0 READ reg=%02X -> %02X\n", val, ret);
        } else if (cmd == 0xC2) {
            solo1_ess_reg_write(dev, 0xC3, val);
            solo1_io_trace("DSP C2 WRITE C3 <- %02X\n", val);
        } else if (cmd == 0xCF) {
            solo1_ess_reg_write(dev, 0xCF, val);
            solo1_io_trace("DSP CF WRITE CF <- %02X\n", val);
        } else if ((cmd >= 0xA0) && (cmd <= 0xBF) && dev->ess_ext_enabled) {
            solo1_ess_reg_write(dev, cmd, val);
            solo1_io_trace("DSP EXT WRITE reg=%02X <- %02X\n", cmd, val);
        } else {
            solo1_io_trace("DSP pending cmd=%02X data=%02X ignored\n", cmd, val);
        }
        return;
    }
    ...
    switch (val) {
        ...
        case 0xC6:
            ...
            solo1_io_trace("DSP EXTENSIONS ENABLED\n");
            break;

        case 0xC7:
            dev->ess_ext_enabled = 0;
            solo1_io_trace("DSP EXTENSIONS DISABLED\n");
            break;
        ...
        case 0xC3: {
            ...
            solo1_sb_fifo_put(dev, ret);
            solo1_io_trace("DSP C3 READ -> %02X\n", ret);
            break;
        }
        ...
        case 0xCE: {
            ...
            solo1_sb_fifo_put(dev, ret);
            solo1_io_trace("DSP CE READ CF -> %02X\n", ret);
            break;
        }
        ...
        default:
            if ((val >= 0xA0) && (val <= 0xBF) && dev->ess_ext_enabled) {
                ...
                solo1_io_trace("DSP UNKNOWN COMMAND %02X\n", val);
            }
            break;
    }
}
...
solo1_sb_read(uint16_t addr, void *priv)
{
    ...
    /*
     * Solo-1 software-reset handshake:
     * after SBBase+6 bit 0 is asserted then cleared, bit 7 of
     * SBBase+Eh becomes set and SBBase+Ah returns AAh.
     */
    if (off == 0x0e) {
        ...
        /*
         * Writes to SBBase+Ch are DSP commands/data.
         * Reads are embedded-processor/write-buffer status.
         */
...
{
    solo1_t *dev = (solo1_t *) priv;
    uint8_t off = (uint8_t) (addr - dev->mpu_base_mapped);
    uint8_t ret;

    if (off == 0x00) {
        /* MPU-401 data port. A reset command FFh produces ACK FEh. */
        if (dev->mpu_ack_pending) {
            ret = 0xfe;
        ...
        /*
         * MPU-401 status:
         * bit 7 clear = command/data output port ready.
         * bit 6 set   = no input byte waiting.
         * While the reset ACK is pending, input-data-ready is asserted.
         */
        ret = dev->mpu_ack_pending ? 0x00 : 0x40;
    } else {
        ret = 0xff;
    }
...
solo1_mpu_write(uint16_t addr, uint8_t val, void *priv)
{
    ...
    if (off < sizeof(dev->mpu_regs))
        dev->mpu_regs[off] = val;

    if (off == 0x01) {
        if (val == 0xff) {
            /* MPU-401 reset: make FEh available at the data port. */
            dev->mpu_ack_pending = 1;
            solo1_io_trace("MPU RESET -> ACK FE pending\n");
        } else if (val == 0x3f) {
            /*
             * UART-mode command. Real MPU-401 hardware acknowledges this
             * with FEh as well; this is enough for driver enumeration.
             */
            dev->mpu_ack_pending = 1;
            solo1_io_trace("MPU UART MODE -> ACK FE pending\n");
        }
    }
...
solo1_update_native_mappings(solo1_t *dev)
{
    ...
    /*
     * Deliberately do NOT connect the PCI-native BARs to the existing
     * AudioDrive core in this diagnostic build.  Phase 3 proved that doing so
     * can terminate 86Box when the Windows 98 ESS driver starts initialization.
     * Legacy 220/240 decoding remains controlled separately by LACR.
     */
    solo1_update_legacy(dev);

    solo1_trace("NATIVE-TRACE PCIIO=%d IO=%04X SB=%04X MPU=%04X GP=%04X IRQ=%u DDMA=%d@%04X\n",
                pci_io, io_base, sb_base, mpu_base, gp_base,
                dev->pci_regs[0x3c], ddma_enable, ddma_base);
}
...
solo1_update_legacy(solo1_t *dev)
{
    ...
    /*
     * Actually expose the Solo-1 legacy resources.
     *
     * This call was accidentally lost when the native PCI BARs were
     * detached from the ESS AudioDrive compatibility core during the
     * phase-3 safety work.  Without it, software can configure/read the
     * PCI legacy registers successfully but there is no DSP at 220/240h
     * for a DOS game to detect.
     *
     * The physical Solo-1's FM legacy resource is 388h.  The helper also
     * maps the SB-base FM aliases when SB/FM are enabled.
     */
    ...
    solo1_trace("LEGACY LA=%d SB=%d@%03X IRQ%d DMA%d FM=%d MPU=%d@%03X IRQ%d GP=%d LACR=%02X%02X CFG50=%02X%02X%02X%02X\n",
                legacy_enable, sb_enable, sb_base, sb_irq, dma,
                fm_enable, mpu_enable, mpu_base,
                mpu_irq_enable ? mpu_irq : -1, gp_enable,
                lacr_hi, lacr_lo,
                dev->pci_regs[0x53], dev->pci_regs[0x52],
                dev->pci_regs[0x51], dev->pci_regs[0x50]);
}
...
solo1_pci_write(int func, int addr, UNUSED(int len), uint8_t val, void *priv)
{
    ...
    uint8_t old = dev->pci_regs[addr];

    solo1_flush_repeated_reads(dev);
    solo1_trace("PCI slot=%02X WRITE func=%d %02X <- %02X (old %02X)\n",
                dev->pci_slot, func, addr, val, old);
    ...
    if (old != dev->pci_regs[addr])
        solo1_trace("PCI slot=%02X write %02X: %02X -> %02X\n",
                    dev->pci_slot, addr, old, dev->pci_regs[addr]);
...
solo1_reset(void *priv)
{
    ...
    solo1_trace("RESET slot=%02X VID=125D DID=1969 LACR=%02X%02X\n",
                dev->pci_slot, dev->pci_regs[0x41], dev->pci_regs[0x40]);
...
static void *
solo1_init(UNUSED(const device_t *info))
{
    solo1_t *dev = calloc(1, sizeof(solo1_t));

    dev->last_read_func = -1;
    dev->last_read_addr = -1;
